...
if [ -f .use-venv ] && [ "$(cat .use-venv)" = "1" ] && [ -x .venv/bin/python ]; then
  ...
  exec env TTS_BACKEND="$TTS_BACKEND" GPU="${GPU:-1}" USE_TALKING_AVATAR="${USE_TALKING_AVATAR:-0}" \
    ...
    VOICE_MATCH_THRESHOLD="${VOICE_MATCH_THRESHOLD:-0.70}" \
    VOICE_WEAK_MATCH="${VOICE_WEAK_MATCH:-0.65}" \
    VOICE_MATCH_MARGIN="${VOICE_MATCH_MARGIN:-0.05}" \
    VOICE_MULTI_MATCH_THRESHOLD="${VOICE_MULTI_MATCH_THRESHOLD:-0.68}" \
    VOICE_MULTI_WEAK_MATCH="${VOICE_MULTI_WEAK_MATCH:-0.63}" \
